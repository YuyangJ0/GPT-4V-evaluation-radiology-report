...
def preprocess_reports(data_path, start, end, sentence=False, image=False):
    ...
    for idx, row in impressions.iterrows():
        ...
        if len(sen) == 0: # if the report is empty
            sen.append('')
    ...
    with open("./tmp/temp_dygie_input.json",'w') as outfile:
...
def run_inference(model_path, cuda):
    ...
    """
    out_path = "./tmp/temp_dygie_output.json"
    data_path = "./tmp/temp_dygie_input.json"
...
def postprocess_reports(data_source, data_split):
    ...
    file_name = f"./tmp/temp_dygie_output.json"
    data = []
