...
# add ROUGE-L, METEOR; change BLEU-2 to BLEU-1 & BLEU-4
...
def add_bleu_1_col(gt_df, pred_df):
    """Computes BLEU-1 and adds scores as a column to prediction df."""
    pred_df["bleu_1"] = [0.0] * len(pred_df)
    for i, row in gt_df.iterrows():
        gt_report = prep_reports([row[REPORT_COL_NAME]])[0]
        predicted_report = prep_reports([pred_df.loc[i][REPORT_COL_NAME]])[0]
        
        score = [sentence_bleu([gt_report], predicted_report, weights=(1, 0, 0, 0))] ### change the weights to change type of bleu
        pred_df.at[i, "bleu_1"] = score[0]

    return pred_df

def add_bleu_4_col(gt_df, pred_df):
    """Computes BLEU-4 and adds scores as a column to prediction df."""
    pred_df["bleu_4"] = [0.0] * len(pred_df)
    for i, row in gt_df.iterrows():
        gt_report = prep_reports([row[REPORT_COL_NAME]])[0]
        predicted_report = prep_reports([pred_df.loc[i][REPORT_COL_NAME]])[0]
        
        score = [sentence_bleu([gt_report], predicted_report, weights=(0.25, 0.25, 0.25, 0.25))] ### change the weights to change type of bleu
        pred_df.at[i, "bleu_4"] = score[0]

    return pred_df

def add_meteor_col(gt_df, pred_df):
    """Computes METEOR and adds scores as a column to the prediction df."""
    pred_df["meteor_score"] = [0.0] * len(pred_df)

    for i, row in gt_df.iterrows():
        gt_report = prep_reports([row[REPORT_COL_NAME]])[0]
        predicted_report = prep_reports([pred_df.loc[i][REPORT_COL_NAME]])[0]
        
        score = single_meteor_score(gt_report, predicted_report)
        pred_df.at[i, "meteor_score"] = score

    return pred_df


def add_rouge_l_col(gt_df, pred_df):
    """Computes ROUGE-L and adds scores as a column to the prediction DataFrame."""
    ...
    for i, row in gt_df.iterrows():
        ...
        gt_text = ' '.join(gt_report)
        predicted_text = ' '.join(predicted_report)
        
        scores = scorer.score(predicted_text, gt_text) 
        pred_df.at[i, "rouge_l"] = scores['rougeL'].fmeasure
...
def calc_metric(gt_csv, pred_csv, out_csv, use_idf=False): # TODO: support single metrics at a time
    ...
    # add bleu-1 column to the eval df
    pred = add_bleu_1_col(gt, pred)

    # add bleu-4 column to the eval df
    pred = add_bleu_4_col(gt, pred)

    # add rouge column to the eval df ###
    pred = add_rouge_l_col(gt, pred)

    # add meteor column to the eval df ###
    pred = add_meteor_col(gt, pred)
